...
class Decoder1(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False, up_type=0):
        ...
        self.extradropout=nn.Dropout3d(p=0.3, inplace=False)
    def forward(self, features):
        ...
        x5 = features[4]
        if self.has_dropout:
            gaosinoise = torch.clamp(0.1 * torch.randn(x5.shape[:]), -0.2, 0.2).to(x5.device)
            x5=self.extradropout(x5)
            x5=x5+gaosinoise
...
class Decoder2(nn.Module):
    ...
    def forward(self, features):
        ...
        x5 = features[4]
        if self.has_dropout:
            gaosinoise5 = torch.clamp(0.1 * torch.randn(x5.shape[:]), -0.2, 0.2).to(x5.device)
            x5=self.extradropout(x5)
            x5=x5+gaosinoise5
        if self.has_dropout:
            gaosinoise2 = torch.clamp(0.1 * torch.randn(x2.shape[:]), -0.2, 0.2).to(x2.device)
            x2=self.extradropout(x2)
            x2=x2+gaosinoise2
        if self.has_dropout:
            gaosinoise4 = torch.clamp(0.1 * torch.randn(x4.shape[:]), -0.2, 0.2).to(x4.device)
            x4=self.extradropout(x4)
            x4=x4+gaosinoise4
...
class Decoder3(nn.Module):
    ...
    def forward(self, features):
        ...
        if self.has_dropout:
            gaosinoise1 = torch.clamp(0.1 * torch.randn(x1.shape[:]), -0.2, 0.2).to(x1.device)
            x1=self.extradropout(x1)
            x1=x1+gaosinoise1
            

        if self.has_dropout:
            gaosinoise3 = torch.clamp(0.1 * torch.randn(x3.shape[:]), -0.2, 0.2).to(x3.device)
            x3=self.extradropout(x3)
            x3=x3+gaosinoise3
...
class VNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False):
        ...
        self.decoder1 = Decoder1(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual, 0)
...
class MDPCT(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False):
        super(MDPCT, self).__init__()
